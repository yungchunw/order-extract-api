...
def load_mapping_list():
    ...
    active_item = pd.read_csv('mapping_list/cust_item_active.txt', delimiter='\t')
...
def gen_supplierName(ori_json, ouID, supplierName_all, supplierName_list, threshold=0.5):
    ...
    if supplier_name_ocr is not None:
        supplier_name_ocr = HanziConv.toTraditional(supplier_name_ocr) # ocr轉換為繁體
...
def gen_custPoNumber(ori_json, custID, custPoNumber_list):
    ...
    if po_num_ocr is not None:
        po_num_ocr = ''.join([item for item in list(dict.fromkeys(po_num_ocr.split(' ')))]) # drop duplicates 
        custPoNumber = ''
    ...
    if addr_ocr is not None:
        ...
        for idx in range(len(address_list[custID]['SITE_USE_CODE'])):
            if address_list[custID]['SITE_USE_CODE'][idx] == code:
                addr = address_list[custID]['ADDRESS2'][idx] + address_list[custID]['ADDRESS3'][idx] + address_list[custID]['ADDRESS4'][idx]
...
def gen_address(ori_json, custID, ouID, address_list, threshold):
    ...
    shipAddr, shipAddr_sim = address_sim(shipAddr_ocr, address_list, custID, 'SHIP_TO')
    
    
    # billAddr
    billAddr_ocr = ori_json['header']['billAddr']
    billAddr, billAddr_sim = address_sim(billAddr_ocr, address_list, custID, 'BILL_TO')
   
    if billAddr == '':
        billAddr, billAddr_sim = address_sim(shipAddr, address_list, custID, 'BILL_TO')
        if billAddr_sim < threshold:
            billAddr = ''
    # deliverAddr
    deliverAddr_ocr = ori_json['header']['deliverAddr']
    deliverAddr, deliverAddr_sim = address_sim(deliverAddr_ocr, address_list, custID, 'DELIVER_TO')
    
    if deliverAddr == '':
        deliverAddr, deliverAddr_sim = address_sim(shipAddr, address_list, custID, 'DELIVER_TO')
...
def gen_sellingPrice(line_ocr):
    ...
    if price_ocr is not None:
        ...
        price_ocr = ''.join([item for item in list(dict.fromkeys(price_ocr.split(' ')))]) # drop duplicates
        sellingPrice = []
...
def gen_voQty(line_ocr):
    ...
    if qty_ocr is not None:
        ...
        qty_ocr = ''.join([item for item in list(dict.fromkeys(qty_ocr.split(' ')))]) # drop duplicates
        #print(qty_ocr)
...
def extract_info(azure_json, file_name):
    ...
    output_json['header']['supplierName'] = gen_supplierName(ori_json, ouID, supplierName_all, supplierName_list)
    output_json['header']['custPoNumber'] = gen_custPoNumber(ori_json, custID, custPoNumber_list)
    # undo
    output_json['header']['poDate'] = ori_json['header']['poDate']
    output_json['header']['shipAddr'], output_json['header']['billAddr'], \
        output_json['header']['deliverAddr'] = gen_address(ori_json, custID, ouID, address_list, 0.6)
    output_json['header']['payCurrency'] = gen_payCurrency(key, payCurrency_list)
    output_json['header']['paymentTerm'] = gen_term(ori_json, custID, ouID, paymentTerm_list, 'paymentTerm')
    output_json['header']['tax'] = gen_term(ori_json, custID, ouID, tax_list, 'tax')
    output_json['header']['tradeTerm'] = gen_term(ori_json, custID, ouID, tradeTerm_list,'tradeTerm')
    ...
    for row in ori_json['line']:
        ...
        line_info['originalRequestDate'] = row['originalRequestDate']
        output_json['line'].append(line_info)
        count += 1
    return output_json
